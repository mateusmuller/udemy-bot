...
class UdemyInstructor:
  ...
  def pull_new_reviews(self):
    ...
    s3.meta.client.download_file('udemy-mateusmuller-s3', 'db.json', '/tmp/db.json')
    s3.meta.client.download_file('udemy-mateusmuller-s3', 'new_reviews.txt', '/tmp/new_reviews.txt')
    s3.meta.client.download_file('udemy-mateusmuller-s3', 'old_reviews.txt', '/tmp/old_reviews.txt')

    with open ("/tmp/db.json", "w") as db:
      json.dump(response.json(), db, indent=2)

    reviews = json.loads(response.content)
    course_ids = '\n'.join(reviews["id"] for reviews in reviews["results"])

    with open("/tmp/new_reviews.txt", "w") as file:
      file.write(course_ids)

  def get_diff_reviews(self):
    with open("/tmp/old_reviews.txt") as old_reviews:
      with open("/tmp/new_reviews.txt") as new_reviews:
        strip_diff = [diff.strip() for diff in set(new_reviews).difference(old_reviews)]
    return strip_diff

  @staticmethod
  def set_old_db():
    with open ("/tmp/new_reviews.txt", "r") as new_reviews:
      with open ("/tmp/old_reviews.txt", "w") as old_reviews:
        old_reviews.write(new_reviews.read())

    s3 = boto3.resource('s3')
    s3.meta.client.upload_file('/tmp/db.json', 'udemy-mateusmuller-s3', 'db.json')
    s3.meta.client.upload_file('/tmp/new_reviews.txt', 'udemy-mateusmuller-s3', 'new_reviews.txt')
    s3.meta.client.upload_file('/tmp/old_reviews.txt', 'udemy-mateusmuller-s3', 'old_reviews.txt')
